 /*  File: get_signif_maf_thrFile_1.c // computes maf distribution over variant positions from vcf files
 ...
 *-------------------------------------------------------------------
 * Description: computes maf distrib over variant positions from just esigtracted 6 fileld
...
input2: esigtracted vcf,


output1: info AF.txt=

pos  %AF Q30frac (for this position)
1546 100 0.89
4469 100 0.86
4532 100 0.78
4559  97 0.95
...
usage:./get_peak name.thr extracted.vcf .infAF .distrAF
...
int main    (int argc, char *argcv[]) {
    ...
    // to find significat peak of AF histo!
      ...
      for(i=0;i<49;i++)//only 49, because we need only lower freq, esp for diploid organisms
      {
       ...
       less25=histAF[0];
	   for(i=0;i<26;i++)//only  because we need only lower freq, esp for diploid organisms
	   {
	   		less25+=histAF[i];
	   }
	   bw25_40=histAF[26];
...
//compute 25% AF skewness
      ...
      // find all percents with non-zero histAF, and then first peak among them
...
// find minimal (first) non-zero peak
	//data is histAF, its indesig is bin=percentage
