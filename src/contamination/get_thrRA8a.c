...
usage:./get_thrRA MIN_DEPTH_R(integer) MIN_DEPTH_A(integer) name.vcfa  ra_i_j.thrD
...
int main    (int argc, char *argcv[]) {
 ...
 // three outputs
    ...
    while( (n = fscanf(extract_vcfFile,"%d,%c,%c,%d,%d,%d,%d,%d", &pos, &ref, &alt,&D, &DP4[0], &DP4[1], &DP4[2], &DP4[3])) >= 0 && firstEightAreOK == 1)// && canWriteAF == 1)
    // read the Read Title
    {
