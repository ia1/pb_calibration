...
int main    (int argc, char *argcv[]) {
    ...
    FILE *extract_vcfaFile, *thrFile, *distribFile, *filtered_vcfaFile; //file handles
    ...
    char ref,alt;//nucleotides from vcf file
    ...
    extract_vcfaFile=fopen(argcv[3],"r");
    if (extract_vcfaFile == NULL) {
      printf("cannot open first input _vcfa.txt file %s\n", argcv[3]);
      return -1;
    }
 // two outputs
     ...
     filtered_vcfaFile=fopen(argcv[6],"w");
		    if (filtered_vcfaFile == NULL) {
    ...
    //8 fields of input file,CSV reading
    while( (n = fscanf(extract_vcfaFile,"%d,%c,%c,%d,%d,%d,%d,%d", &pos, &ref,&alt,&D, &DP4[0], &DP4[1], &DP4[2], &DP4[3])) >= 0 && firstSixAreOK == 1)// && canWriteAF == 1)
    // read the Read Title
    {
        if( n != NRID )     // incorrect format
        {
           ...
           if ( DP4[0] >= thR && DP4[1]>= thR && DP4[2]>= thA && DP4[3]>= thA)// && D < 10)// 0 ref are ok
           {
             ...
             if( fprintf(filtered_vcfaFile,"%d,%d,%d,%d,%d,%d,%d,%d\n",pos,ref_num,alt_num, D, DP4[0], DP4[1],DP4[2], DP4[3]) <= 0 ) {
			          canWriteDistrib = 0;
 ...
 // fill in the output files
    ...
    fclose(extract_vcfaFile);
    fclose(thrFile);
    fclose(distribFile);
    fclose(filtered_vcfaFile);
