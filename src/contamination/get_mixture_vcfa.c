...
#define NRID 8      // Number of variant ids=fields: pos, ref, alt,Depth,DP4,(no PV4+maf) in esigtracted_vcf files
...
//-----------------------------------------open files for read/write
  ...
  printf("ploidy= %d\n", plo);
	...
	get_skew_ploidy	(thr_sk, plo);
	  ...
	  //8 fields of input2 file	// no PV4: do it to compute AAF=AF, but num Ref Alt


    while( (n = fscanf(extract_vcfFile,"%d,%d,%d,%d,%d,%d,%d,%d", &pos, &ref,&alt,&D, &DP4[0], &DP4[1], &DP4[2], &DP4[3])) >= 0 && firstEightAreOK == 1 && canWriteMix == 1)
